...
from vecxoz_utils import convert_proba
...
parser.add_str('--data_tfrec_dir',    default='../../data/data-tfrec-frames024-step001-lstm', help='Data directory containig TFRecord files')
...
parser.add_str('--metric_name',       default='rmseclas',        help='Metric name')
parser.add_str('--monitor',           default='val_rmseclas',    help='Value to monitor during training to apply checkpoint callback, etc.')
...
parser.add_int('--n_epochs',          default=15,            help='Number of epochs to train')
...
parser.add_bool('--pretrained',       default=True,          help='Whether to use pretrained weights. Some models require this arg in addition to weights arg')
...
parser.add_int('--n_classes',         default=156,             help='Number of classes for classification task. For regression task must be 1')
parser.add_float('--label_smoothing', default=0.3,           help='Label smoothing. Apllicable to classification task only')
...
# Model function
args.model_func = efn.EfficientNetB3
...
feature_description = {
    'image': tf.io.FixedLenFeature([args.n_channels_total], tf.string),
    'label_le': tf.io.FixedLenFeature([], tf.int64),
}
...
def parse_example(example_proto):
    ...
    #
    # fn = lambda image_encoded: tf.image.decode_jpeg(image_encoded, channels=1, dct_method='INTEGER_ACCURATE')
    image = tf.map_fn(fn, d['image'], dtype=tf.uint8)                                   # (24, 366, 366, 1)
    image = tf.squeeze(image)                                                           # (24, 366, 366)
    image = tf.reshape(image, [args.volume_size, args.n_channels, args.dim, args.dim])  # (8, 3, 366, 366)
    image = tf.transpose(image, perm=[0, 2, 3, 1])                                      # (8, 366, 366, 3)
    #
    label = tf.cast(d['label_le'], tf.int32)
...
class RMSECLAS(tf.keras.metrics.RootMeanSquaredError):
    """
    Compute RMSE from classification probabilities
    """
    def __init__(self, name='rmseclas', **kwargs):
        super(RMSECLAS, self).__init__(name=name, **kwargs)
    #
    def update_state(self, y_true, y_pred, sample_weight=None):
        super(RMSECLAS, self).update_state(tf.argmax(y_true, axis=1), tf.argmax(y_pred, axis=1), sample_weight=sample_weight)
    #
    def result(self):
        return super(RMSECLAS, self).result()
...
def init_model(print_summary=True):
    ...
    inp = tf.keras.layers.Input(shape=(args.volume_size, 
                                       args.dim, 
                                       args.dim, 
                                       args.n_channels), dtype=tf.float32)  # TensorShape([None, 8, 366, 366, 3])
    effnet = args.model_func(weights=args.weights, include_top=False)
    pool_2d = tf.keras.layers.GlobalAveragePooling2D()
    #
    x = tf.keras.layers.TimeDistributed(effnet)(inp)                        # TensorShape([None, 8, 12, 12, 1280])
    x = tf.keras.layers.TimeDistributed(pool_2d)(x)                         # TensorShape([None, 8, 1280])
    x = tf.keras.layers.LSTM(1024, return_sequences=False)(x)               # TensorShape([None, 1024])
    x = tf.keras.layers.Dense(300, activation='relu')(x)                    # TensorShape([None, 300])
    out = tf.keras.layers.Dense(args.n_classes, activation='softmax')(x)    # TensorShape([None, 156])
    #
    model = tf.keras.models.Model(inp, out, name='model')
    #
    model.compile(optimizer=tf.keras.optimizers.Adam(args.lr), 
                  loss=tf.keras.losses.CategoricalCrossentropy(from_logits=False, label_smoothing=args.label_smoothing),
                  metrics=[RMSECLAS(name=args.metric_name), 'acc'])
    if print_summary:
        model.summary()
    return model
...
if 'train' in args.job or 'val' in args.job or 'test' in args.job:
    preds_dir_proba = convert_proba(args.data_dir, args.data_preds_dir, args.n_folds, args.tta_number)
