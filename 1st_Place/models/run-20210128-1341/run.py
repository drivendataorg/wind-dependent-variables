...
parser.add_str('--data_tfrec_dir',    default='../../data/data-tfrec-frames060-step001-lstm', help='Data directory containig TFRecord files')
...
parser.add_int('--n_channels_total',  default=60,            help='Total number of monochrome frames to read from TFRecord. Applicable to architectures trained on sequences (volumes) of 3-channel images')
parser.add_int('--volume_size',       default=20,             help='Number of 3-channel images in a sequence (volume)')
...
# Model function
args.model_func = tf.keras.applications.ResNet50
...
def parse_example(example_proto):
    ...
    image = tf.map_fn(fn, d['image'], dtype=tf.uint8)                                   # (60, 366, 366, 1)
    image = tf.squeeze(image)                                                           # (60, 366, 366)
    image = tf.reshape(image, [args.volume_size, args.n_channels, args.dim, args.dim])  # (20, 3, 366, 366)
    image = tf.transpose(image, perm=[0, 2, 3, 1])                                      # (20, 366, 366, 3)
    #
