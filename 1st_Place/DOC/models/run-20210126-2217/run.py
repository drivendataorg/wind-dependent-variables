...
parser.add_int('--volume_size',       default=9,             help='Number of 3-channel images in a sequence (volume)')
...
def parse_example(example_proto):
    ...
    image = tf.map_fn(fn, d['image'][::-1][:27][::-1], dtype=tf.uint8)                  # (27, 366, 366, 1)
    image = tf.squeeze(image)                                                           # (27, 366, 366)
    image = tf.reshape(image, [args.volume_size, args.n_channels, args.dim, args.dim])  # (9, 3, 366, 366)
    image = tf.transpose(image, perm=[0, 2, 3, 1])                                      # (9, 366, 366, 3)
    #
