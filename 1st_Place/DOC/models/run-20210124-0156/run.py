...
parser.add_int('--n_epochs',          default=30,            help='Number of epochs to train')
parser.add_int('--batch_size',        default=32,           help='Batch size')
parser.add_float('--lr',              default=1e-5,          help='Learning rate')
...
parser.add_int('--buffer_size',       default=128,          help='Shuffle buffer size for tf.data. For small RAM or large data use small buffer e.g. 128 or None to disable')
...
class TransformerBlock(tf.keras.layers.Layer):
    """
    Transformer block (Encoder layer) with tf.keras.layers.MultiHeadAttention()
    Transformer input and output shape is 3D, equivalent to LSTM(return_sequences=True)
    Mask is not used in call to MultiHeadAttention
    MultiHeadAttention is present since TF 2.4.0
    If query, key, value are the same, then this is self-attention.    
    --
    https://tf.keras.io/examples/nlp/text_classification_with_transformer
...
def init_model(print_summary=True):
    ...
    dense = tf.keras.layers.Dense(embed_dim, activation='relu')
    encoder_layers = [TransformerBlock(embed_dim=embed_dim, num_heads=num_heads, ff_dim=ff_dim) for _ in range(n_layers)]
    #
    ...
    x = tf.keras.layers.TimeDistributed(dense)(x)                           # TensorShape([None, 8, 512])
    #
    for i in range(n_layers):
        x = encoder_layers[i](x)                                            # TensorShape([None, 8, 512])
    #
    x = tf.keras.layers.GlobalAveragePooling1D()(x)                         # TensorShape([None, 512])
    out = tf.keras.layers.Dense(args.n_classes, activation='linear')(x)     # TensorShape([None, 1])
    #
