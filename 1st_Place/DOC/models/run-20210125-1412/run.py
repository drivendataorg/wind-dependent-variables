...
parser.add_int('--volume_size',       default=12,             help='Number of 3-channel images in a sequence (volume)')
...
def parse_example(example_proto):
    ...
    image = tf.map_fn(fn, d['image'][::-1][:36][::-1], dtype=tf.uint8)                  # (36, 366, 366, 1)
    image = tf.squeeze(image)                                                           # (36, 366, 366)
    image = tf.reshape(image, [args.volume_size, args.n_channels, args.dim, args.dim])  # (12, 3, 366, 366)
    image = tf.transpose(image, perm=[0, 2, 3, 1])                                      # (12, 366, 366, 3)
    #
