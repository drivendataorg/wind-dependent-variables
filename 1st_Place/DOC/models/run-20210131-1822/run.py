...
def init_model(print_summary=True):
    ...
    model_interim = args.model_func(input_shape=(args.dim, 
                                                 args.dim, 
                                                 args.n_channels),
                                    weights=args.weights, include_top=False)
    pool_2d = tf.keras.layers.GlobalAveragePooling2D()
    #
    x = tf.keras.layers.TimeDistributed(model_interim)(inp)                 # TensorShape([None, 8, 12, 12, 2048])
    x = tf.keras.layers.ConvLSTM2D(512, 3, 3, return_sequences=True)(x)     # TensorShape([None, 8, 4, 4, 512])
    x = tf.keras.layers.TimeDistributed(pool_2d)(x)                         # TensorShape([None, 8, 512])
