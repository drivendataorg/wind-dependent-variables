...
def init_model(print_summary=True):
    #
    inp = tf.keras.layers.Input(shape=(args.dim, args.dim, args.n_channels), dtype=tf.float32)
    #
    effnet = args.model_func(input_shape=(args.dim, args.dim, args.n_channels), weights=args.weights, include_top=False)
    #
    x = effnet(inp)
