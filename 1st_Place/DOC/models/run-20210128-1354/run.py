...
parser.add_int('--dim',               default=112,           help='Image size in pixels. This final size which will be passed to a model')
parser.add_int('--dim_before_aug',    default=112,           help='Image size in pixels before augmentation. This may differ form final size passed to a model')
...
def parse_example(example_proto):
    ...
    image = tf.map_fn(fn, d['image'], dtype=tf.uint8)                                   # (48, 112, 112, 1)
    image = tf.squeeze(image)                                                           # (48, 112, 112)
    image = tf.reshape(image, [args.volume_size, args.n_channels, args.dim, args.dim])  # (16, 3, 112, 112)
    image = tf.transpose(image, perm=[0, 2, 3, 1])                                      # (16, 112, 112, 3)
    #
...
def init_model(print_summary=True):
    model_inner = create_model_sequential()
    model_inner.load_weights('../../weights_3rd_party/C3D_Sport1M_weights_keras_2.2.4.h5')
    #
    out = Dense(args.n_classes, activation='linear', name='dense_last_new')(model_inner.layers[-2].output)
    model = Model(inputs=model_inner.input, outputs=out)
    #
