...
parser.add_str('--data_tfrec_dir',    default='../../data/data-tfrec-frames096-step001-lstm', help='Data directory containig TFRecord files')
...
parser.add_int('--n_channels_total',  default=96,            help='Total number of monochrome frames to read from TFRecord. Applicable to architectures trained on sequences (volumes) of 3-channel images')
parser.add_int('--volume_size',       default=32,             help='Number of 3-channel images in a sequence (volume)')
...
def parse_example(example_proto):
    ...
    image = tf.map_fn(fn, d['image'], dtype=tf.uint8)                                   # (96, 366, 366, 1)
    image = tf.squeeze(image)                                                           # (96, 366, 366)
    image = tf.reshape(image, [args.volume_size, args.n_channels, args.dim, args.dim])  # (32, 3, 366, 366)
    image = tf.transpose(image, perm=[0, 2, 3, 1])                                      # (32, 366, 366, 3)
    #
