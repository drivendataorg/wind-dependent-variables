...
def init_model(print_summary=True):
    ...
    inp = tf.keras.layers.Input(shape=(args.volume_size, 
                                       ...
                                       args.dim, 
                                       args.n_channels), 
                                dtype=tf.float32)                           # TensorShape([None, 8, 366, 366, 3])
    sub_model = args.model_func(input_shape=(args.volume_size, 
                                             args.dim, 
                                             args.dim, 
                                             args.n_channels), 
                                weights=args.weights, include_top=False)
    #
    x = sub_model(inp)                                                      # TensorShape([None, 1, 12, 12, 2048])
    x = tf.keras.layers.GlobalAveragePooling3D()(x)                         # TensorShape([None, 2048])
