...
MODEL_TYPE = 'L1A'
MODEL_VERSION = 'A'
TRAINING_VERSION = '3'  # vgg11_gru_s24B - CS061bn24
...
BASEMODEL_FEATURES = {
    'vgg11_bn': (25088, lambda model: nn.Sequential(*list(model.children())[1:-7], nn.Flatten())),  #Remove AvgPool
}
...
def get_dataloaders(path_to_data, datasets, args=ARGS):
    ...
    storms_data_reader = {
        'train': StormsDatasetSequence(datasets['train'], nb_imgs=args.sequence_length, gap=args.sequence_gap,
                                       missing='black', max_block_size=5),
...
class NNModel(nn.Module):
    def __init__(self, basemodel, num_ftrs, num_channels):
        ...
        hidden_size = 1024
        num_layers = 1
        self.gru = nn.GRU(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, batch_first=True)  # lstm
...
def get_learner(nb_classes, all=False, args=ARGS):
    ...
    if all:
        ...
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=3, verbose=True, threshold=0.001,
                                      min_lr=1e-5)
        from src.pytorch.early_stoppers import EarlyStopping
        early_stopper = EarlyStopping(patience=6, verbose=True, delta=0.001, save_model_path=None, wait=1)
