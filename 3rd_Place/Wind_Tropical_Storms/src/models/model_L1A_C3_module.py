    ...
    'transformations': {
        'train': transforms.Compose([
            ...
            transforms.RandomChoice([
                RandomCircleMask(center_delta=(-0.1, 0.1), radius=(0.6, 0.8), p=0.9, invert_p=1.0),  # center
                RandomCircleMask(center_delta=(-0.1, 0.1), radius=(0.5, 0.7), p=0.9, invert_p=0.0),  # perimeter
            ]),
        ]),
...
class NNModel(nn.Module):
    def __init__(self, basemodel, num_ftrs, num_channels):
        ...
        self.ch_adap = nn.Sequential(*[
            nn.Conv2d(in_channels=self.num_channels, out_channels=3, kernel_size=3, padding=1)
        ])
