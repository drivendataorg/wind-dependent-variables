...
MODEL_TYPE = 'L1A'
MODEL_VERSION = 'A'
TRAINING_VERSION = '7' # vgg11_transformer_s24g10r - CS061tbn24g10r3r1
...
class NNModel(nn.Module):
    def __init__(self, basemodel, num_ftrs, seq_length):
        ...
        emb_dim = 1024
        self.features_to_embedding = nn.Linear(num_ftrs, emb_dim)

        self.pos_embedding = nn.Parameter(torch.randn(1, self.seq_length, emb_dim))
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 3, sx, sy)
        x = self.features(x)
        x = self.features_to_embedding(x)
        x = x.view(bs, ss, -1)
