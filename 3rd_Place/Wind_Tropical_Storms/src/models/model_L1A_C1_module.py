...
def get_dataloaders(path_to_data, datasets, args=ARGS):
    ...
    ## Data reader classes
    storms_data_reader = {
        'train': StormsDatasetRGBSequence(datasets['train'], gap=args.sequence_gap),
        'valid': StormsDatasetRGBSequence(datasets['valid'], gap=args.sequence_gap),
        'fold': StormsDatasetRGBSequence(datasets['fold'], gap=args.sequence_gap),
        'test': StormsDatasetRGBSequence(datasets['test'], gap=args.sequence_gap),
    }
...
def get_learner(nb_classes, args=ARGS):
    ...
    criterion = pytorch_metrics.SmoothL1Loss(beta=4)
