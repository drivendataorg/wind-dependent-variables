...
# Data
args.update({
    'preprocessed': 'dense_optical_flow',
})
# Train ALL
args.update({
...
BASEMODEL_FEATURES = {
    'vgg16_bn': (41472, lambda model: nn.Sequential(*list(model.children())[1:-7], nn.Flatten())),
}
...
def get_dset(path_to_data, args=ARGS):
    ...
    if args.preprocessed is not None:
        ...
        dset_df['image_path'] = dset_df.apply(lambda irow: os.path.join(path_to_data, args.preprocessed, irow.image_id + ".jpg"), axis=1)
