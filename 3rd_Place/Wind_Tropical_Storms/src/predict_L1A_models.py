...
def main(model_id, folds, debug, only_test):
    ...
    # Init logger
    if folds is None:
        log_filename = os.path.join(PATH_TO_ROOT, PATH_TO_LOG, prefix + MODEL_NAME + '_predict.log')
    else:
        log_filename = os.path.join(PATH_TO_ROOT, PATH_TO_LOG, prefix + MODEL_NAME + f'_{folds}_predict.log')
