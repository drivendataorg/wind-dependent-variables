...
class WindDataset(Dataset):
    ...
    def drop_aug(self, x):
        idx_arr = list(np.arange(x.shape[-1]))
        if (self.split_type == 'train') and (np.random.rand() > 0.5):
            ...
            #    #print('0', len(idx_arr))
            #else:
            #    idx_arr = idx_arr[:j+1] + idx_arr[j:-2]
            #    #print('1', len(idx_arr))
        else:
            idx_arr = idx_arr[:-1]
...
class Conv(nn.Module):
    def __init__(self, ni, no, kernel, pad):
        super().__init__()
        self.pad = pad
        self.layer1 = nn.Sequential(nn.Conv1d(ni, no, kernel, 1, pad), nn.ReLU())
        self.layer2 = nn.Sequential(nn.Conv1d(no, no, kernel, 1, pad), nn.ReLU())#, nn.ReLU())
...
for train_index, val_index in group_kfold.split(train_feats, train_targets, train_storm_ids):
    ...
    model = ConvNet(train_feats.shape[-1], 9, 1)
    ...
    model_ft_arr, sc, _ = train_model_snapshot(model, criterion, eval_criterion, 0.001, dataloaders, dataset_sizes, device,
...
sub.to_csv('ts_cnn_dain_%dts_after_%s_and_img_feat_%s.csv'%(time_steps, out_path_arr[path_idx], img_out_path_arr[img_path_idx]), index = False)
np.save('ts_cnn_dain_%dts_after_%s_and_img_feat_%s'%(time_steps, out_path_arr[path_idx], img_out_path_arr[img_path_idx]), test_final_pred)
np.save('ts_cnn_dain_%dts_after_%s_and_img_feat_%s_val'%(time_steps, out_path_arr[path_idx], img_out_path_arr[img_path_idx]), val_pred)
