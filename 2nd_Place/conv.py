...
for train_index, val_index in group_kfold.split(train_df, train_df['wind_speed'], train_df['storm_id']):
    ...
    image_datasets = {'train': WindDataset(train_imgs, train_df['wind_speed'].values, 'train', train_index, data_transforms['train']),
                      'val': WindDataset(train_imgs, train_df['wind_speed'].values, 'val', val_index, data_transforms['val'])}

    dataloaders = {'train': torch.utils.data.DataLoader(image_datasets['train'], batch_size=64, shuffle=True, num_workers=8),
    ...
    model_ft_arr, _, _ = train_model_snapshot(model_ft, criterion, eval_criterion, 0.0002, dataloaders, dataset_sizes, device,
                           num_cycles=2, num_epochs_per_cycle=10) #rmse 8.87
...
for _, val_index in group_kfold.split(train_df, train_df['wind_speed'], train_df['storm_id']):
    val_imgs = [train_imgs[idx] for idx in val_index]
    
    val_dataset = WindTestDataset(val_imgs, data_transforms['val'])

    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=16, shuffle=False, num_workers=4)
    
    val_pred[val_index] = test_augment(model_ft, models_w_arr[fold*2:(fold+1)*2], val_dataloader, len(val_dataset), device)
...
train_features = np.zeros((train_df.shape[0], 2048), dtype = np.float32)
...
for _, val_index in group_kfold.split(train_df, train_df['wind_speed'], train_df['storm_id']):
    ...
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=16, shuffle=False, num_workers=8)
    
    train_features[val_index] = test_augment_feat(encapsulated_model_ft, models_w_arr[fold*2:(fold+1)*2], val_dataloader, len(val_dataset), device)
...
np.save(path+'/train_features', train_features)


# In[ ]:


del train_features, train_imgs
