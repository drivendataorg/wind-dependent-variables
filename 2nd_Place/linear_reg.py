...
def train_model_snapshot(model, criterion, eval_criterion, lr, dataloaders, dataset_sizes, device, num_cycles, num_epochs_per_cycle):
    ...
    for cycle in range(num_cycles):
        ...
        optimizer = optim.Adam([#{'params': model.fc1.parameters()},
...
class FC(nn.Module):
    def __init__(self, ni, no):
        ...
        layers = [nn.Linear(ni, no), nn.ReLU()]
...
class MLP(nn.Module):
    def __init__(self, in_ch, n_outputs, p=0.05):
        ...
        self.dean = DAIN_Layer(input_dim = 1, mode='adaptive_avg', mean_lr=0.0001, gate_lr=0.01, scale_lr=0.001)
...
for _, storm_df in df.groupby('storm_id'):
    ...
    for i in range(n):
        ...
        temp[:-1, 0] = temp[1:, 0] - temp[:-1, 0] / np.abs(temp[1:, 1] - temp[:-1, 1] + 1e-6)
        if i < time_steps:
...
for train_index, val_index in group_kfold.split(train_feats, train_targets, train_storm_ids):
    ...
    model = MLP(train_feats.shape[-1], 1)
...
sub.to_csv('linear_dain_%dts_after_%s_and_img_feat_res50.csv'%(time_steps, out_path_arr[path_idx]), index = False)
np.save('linear_dain_%dts_after_%s_and_img_feat_res50'%(time_steps, out_path_arr[path_idx]), test_final_pred)
np.save('linear_dain_%dts_after_%s_and_img_feat_res50_val'%(time_steps, out_path_arr[path_idx]), val_pred)
