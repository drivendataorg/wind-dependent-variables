...
class WindDataset(Dataset):
    ...
    def create_seq(self, i):
        idx_arr = [i]
        for j in range(i-step_size, i-time_steps-1, -1*step_size):
        ...
        img_arr = [Image.fromarray(np.array(img_arr[j:j+3]).transpose(1,2,0)[:,:,ch_arr]) for j in range(0, time_steps//step_size, 3)]
        return img_arr
...
class WindTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_arr = [Image.fromarray(self.imgs[idx][:,:,i:i+3][:,:,ch_arr]) for i in range(0, time_steps//step_size, 3)]
...
class WindValDataset(Dataset):
    ...
    def create_seq(self, i):
        idx_arr = [i]
        for j in range(i-step_size, i-time_steps, -1*step_size):
            if self.storm_ids[j] == self.storm_ids[i]:
                idx_arr.append(j)
            else:
                idx_arr.append(idx_arr[-1])
...
path = '6ts_2space_imgs_time_aug_resnet50_models'
os.makedirs(path)
...
#path = '6ts_imgs_time_aug_resnet50_models'
#models_w_arr = []
#for fold in range(5):
...
test_imgs = np.load('6ts_2space_test_imgs_224.npy')
test_imgs_ids = np.load('6ts_2space_test_imgs_ids.npy')
...
sub.to_csv('6ts_2space_imgs_res50_group_5folds_aug_time.csv', index = False)#_%d%d%d.csv'%(ch_arr[0], ch_arr[1], ch_arr[2]), index = False)
