...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        self.fc2 = nn.Sequential(nn.Linear(64*time_steps//3, 64), nn.ReLU(), nn.Linear(64, 1))
...
path = '12ts_imgs_resnet18_models'
os.makedirs(path)
...
for train_index, val_index in group_kfold.split(train_df, train_df['wind_speed'], train_df['storm_id']):
    ...
    dataloaders = {'train': torch.utils.data.DataLoader(image_datasets['train'], batch_size=64, shuffle=True, num_workers=12),
    ...
    model_ft_arr, _, _ = train_model_snapshot(model_ft, criterion, eval_criterion, 0.0005, dataloaders, dataset_sizes, device,
