...
def train_model_snapshot(model, criterion, eval_criterion, lr, dataloaders, dataset_sizes, device, num_cycles, num_epochs_per_cycle):
    ...
    for cycle in range(num_cycles):
        ...
        optimizer = optim.Adam([{'params': model.rnn1.parameters()},
...
class RecurrentNet(nn.Module):
    def __init__(self, in_ch, n_outputs, p=0.05):
        ...
        self.rnn1 = nn.GRU(in_ch, 128, num_layers=2, bidirectional=True, batch_first=True)
...
for train_index, val_index in group_kfold.split(train_feats, train_targets, train_storm_ids):
    ...
    model = RecurrentNet(train_feats.shape[-1], 1)
...
sub.to_csv('gru_dain_%dts_after_%s_and_img_feat_%s.csv'%(time_steps, out_path_arr[path_idx], img_out_path_arr[img_path_idx]), index = False)
np.save('gru_dain_%dts_after_%s_and_img_feat_%s'%(time_steps, out_path_arr[path_idx], img_out_path_arr[img_path_idx]), test_final_pred)
np.save('gru_dain_%dts_after_%s_and_img_feat_%s_val'%(time_steps, out_path_arr[path_idx], img_out_path_arr[img_path_idx]), val_pred)
